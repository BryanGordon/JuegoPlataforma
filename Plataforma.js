...
var jugador= function(){
    ...
    this.vy=0;
    this.vx=0;
    this.gravedad=0.5; ///valores de entorno
    this.friccion=0.5; /// valores de entorno
    this.salto=10;   /// valores de personaje
    this.velocidad=2;   /// valores de personaje
    this.velocidadMax=5; ///valores de personaje
    this.suelo=false;
    ...
    this.fisica=function(){
        ...
        if(this.vx>0){
            if((this.colision(this.x+anchoFicha+this.vx,this.y+1)==true) || (this.colision(this.x+anchoFicha+this.vx,this.y+altoFicha-1)==true)){
                if(this.x!= parseInt(this.x/anchoFicha)*anchoFicha)
                    this.correccion(4);
                this.vx=0;
            }
        }
...
function creaBloque(x,y){
    ...
    var xBloque=parseInt(x/anchoFicha);
    var yBloque=parseInt(y/altoFicha);
...
function dibujaCuadricula(x,y){

    ctx.fillStyle='#777777';
    ctx.fillRect(parseInt(x/anchoFicha)*anchoFicha,parseInt(y/altoFicha)*altoFicha,anchoFicha,altoFicha);
}
...
/*
    ...
    canvas.addEventListener('mousemove',moverRaton,false);
*/
...
function principal(){
    ...
    borrarCanvas();
    dibujaEscenario();
    //dibujaCuadricula(ratonX,ratonY);
    protagonista.dibuja();
}
